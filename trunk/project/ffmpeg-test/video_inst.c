...
video_data_t *openMovie(const char *fileName){
    ...
    err = avformat_open_input(&ctx->p_format_ctx, fileName, NULL, 0);
    if(err != 0){
    ...
    err = avformat_find_stream_info(ctx->p_format_ctx, 0);
    if(err < 0){
...
#ifdef DEBUG 
    av_dump_format(ctx->p_format_ctx, 0, fileName, 0);
#endif
    // Only supporting audio and video for this demo.
    for(i = 0 ; i < ctx->p_format_ctx->nb_streams ; i ++) {
        if(ctx->p_format_ctx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO)
            ctx->idx_video_stream = i;
        if(ctx->p_format_ctx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO)
            ctx->idx_audio_stream = i;
    }
    
    if(ctx->idx_video_stream < 0){
        PMSG1("\n\rvideo_inst.c::openMovie failed to find a video stream.\n");
        avformat_close_input(&ctx->p_format_ctx);
        free(vdata->context);
        free(vdata);
        return NULL;
    }
   ...
   // ctx->p_audio_ctx   = ctx->p_format_ctx->streams[ctx->idx_audio_stream]->codec;
    ...
    if(avcodec_open2(ctx->p_video_ctx,ctx->p_codec,0)<0){
...
void  closeMovie  (video_data_t* vInst) {
    ...
    usleep(10000);
    avformat_close_input(&vInst->context->p_format_ctx);
